...
out_channel = '' // add default out_channel
...
async def on_message(message):
    ...
    if message.content == "too much F now" and message.author.id == 0:
        motor_functions[0] = 1
    if message.content == "continue rr" and message.author.id == 0:
        motor_functions[0] = 0
...
client.run('bot_token') // add a bot token
